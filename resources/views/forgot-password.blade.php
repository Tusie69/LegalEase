@extends('layouts.app', ['title' => 'Forgot password · LegalEase'])
    ...
    <div class="flex flex-1 items-center justify-center px-6 py-16 md:py-20">
        <div class="w-full max-w-[440px]">
            @if (session('reset_link_sent'))
                <h1 class="font-display text-[36px] font-medium tracking-[-0.02em] md:text-[44px]">Check your email</h1>
                <p class="mt-4 text-[15px] text-secondary">
                    If an account exists for <span class="text-text">{{ session('reset_link_sent') }}</span>, we've sent a link to reset your password. The link expires in 60 minutes.
                </p>
            ...
            @else
                <h1 class="font-display text-[36px] font-medium tracking-[-0.02em] md:text-[44px]">Forgot password?</h1>
                <p class="mt-2 text-[15px] text-secondary">Enter the email tied to your account and we'll send a link to set a new one.</p>
                ...
                <form method="POST" action="{{ route('password.email') }}" class="mt-8 space-y-5" novalidate
                    ...
                    <x-button variant="primary" type="submit" class="w-full">Send reset link</x-button>

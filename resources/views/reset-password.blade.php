@extends('layouts.app', ['title' => 'Reset password · LegalEase'])
...
<section class="relative -mt-[72px] flex min-h-screen flex-col md:flex-row">
    ...
    <div class="flex flex-1 items-center justify-center px-6 py-16 md:py-20">
        <div class="w-full max-w-[440px]">
            <h1 class="font-display text-[36px] font-medium tracking-[-0.02em] md:text-[44px]">Set a new password</h1>
            <p class="mt-2 text-[15px] text-secondary">Choose a password you don't use anywhere else. You'll be logged in once it's saved.</p>
            ...
            <form method="POST" action="{{ route('password.update') }}" class="mt-8 space-y-5" novalidate
                ...
                <x-button variant="primary" type="submit" class="w-full">Save new password</x-button>

                <p class="text-center text-[14px] text-muted">
                    <a href="{{ route('login') }}" class="text-text transition-colors hover:text-accent">Back to login</a>
                </p>
            </form>
        </div>
    </div>
